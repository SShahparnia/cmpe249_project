...
**Risk 3 — someone has already done this and you have not found them.** Search coverage for benchmarking papers is genuinely harder than for method papers, because they are often published in workshops, as technical reports, or as bare GitHub repositories. **Before Week 3, search specifically for: "exploration benchmark ROS 2", "frontier exploration comparison study", the CMU exploration benchmark environments, and any IROS/ICRA workshop on exploration benchmarking from 2023–2026.** If a close prior study exists, the project is not dead — it is repositioned as a replication plus the embedded dimension, which is still defensible, but the framing must change and it must change early.

**Risk 4 — the Jetson leg is the novelty and it is scheduled last.** Week 12 is late for the highest-value contribution. Mitigation: three borrowed boards mean bring-up can start in Week 1 in parallel; do that, and get a trivial "hello ROS 2 on the Jetson" working long before Week 12 so that Week 12 is profiling, not installation.
...
- Jetson bring-up starts Week 1 in parallel across all three boards, not Week 12.
